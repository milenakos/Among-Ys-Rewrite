...
class Crew(pygame.sprite.Sprite):
	def __init__(self, colors, name):
		pygame.sprite.Sprite.__init__(self)
		self.image = pygame.image.load(utils.get_path("img/crew/" + colors + ".png")).convert_alpha()
...
class Bot(pygame.sprite.Sprite):
	def __init__(self, idd, colors, names, ticks):
		pygame.sprite.Sprite.__init__(self)
		self.image = pygame.image.load(utils.get_path("img/crew/" + random.choice(colors) + ".png")).convert_alpha()
		self.image = pygame.transform.scale(self.image, (74, 99))
		self.rect = self.image.get_rect()
		self.rect.center = (640, 360)
		self.moves = ast.literal_eval(open(utils.get_path("moves\\file" + str(idd) + ".txt"), "r").read())
			...
			if event.type == pygame.QUIT:
				...
				if event.button == 1:
					try:
						...
						pass
					if kill_btn.collidepoint(event.pos) and not chat_opened and not is_multiplayer:
